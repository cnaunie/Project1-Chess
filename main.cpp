 ...
 * Author: Csatch
 *
 * Created on March 23, 2021, 11:37 PM
 */
...
void setAtPositions(GameBoard&, vector<Piece>&);
...
int main()
{
    while (true) {
        string command;
        system("cls");
        cout << "Welcome to my Chess Game! Ready to play?!" << endl;

            cout << "Enter one of the commands: " << endl;
            cout << "New Game, Load " << endl;
            getline(cin, command);
...
void playGame(vector <Piece>& pieces, int currentPlayer) {
    ...
    while (true) {
        ...
        cout << "Player " << player << " It's your turn! Please enter your move in the following format \"Move d2 to d3\": " << endl;
        ...
        if (move == "Exit" || move == "exit") {
            ...
            if (check == 1 && check == currentPlayer) {
                    ...
                    cout << "Player " << player << " has been checkmated!";
                    Sleep(2000);
                    break;
...
void setAtPositions(GameBoard &board, vector <Piece> &pieces) {
